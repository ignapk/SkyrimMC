...
import com.horus.skyrimmc.gui.GuiSkyrimSmelter;
...
public class BlockSkyrimSmelter extends Block
{
    ...
    public BlockSkyrimSmelter(String name, final Material materialIn) {
        super(materialIn);
    ...
    public boolean onBlockActivated(final World worldIn, final BlockPos pos, final IBlockState state, final EntityPlayer playerIn, final EnumHand hand, final EnumFacing facing, final float hitX, final float hitY, final float hitZ) {
        if (!worldIn.isRemote) {
            Minecraft.getMinecraft().displayGuiScreen((GuiScreen)new GuiSkyrimSmelter(playerIn));
            
            //playerIn.displayGui(new BlockSmelter.Smelter(worldIn, pos)); <-- vanilla 1.12.2 code
        }
        return true;
    }
