...
// TODO: Fix inventory...
public class GuiSkyrimShop extends GuiScreen
{
    private List<ItemStack> allItems;
    //private LinkedHashMap<String, List<ItemStack>> inventory;
    private LinkedHashMap<String, List<ItemStack>> stock;
    ...
    public GuiSkyrimShop(EntityPlayer player, ItemStack[] items) {
        this.player = player;
...
//        }
        ...
        // Show items
        currentItemStacks = (List<ItemStack>) getElementByIndex(stock, categoryIndex);
        for(int i = 0; i < currentItemStacks.size(); i++) {
            ItemStack is = currentItemStacks.get(i);
            String displayName = is.getDisplayName();
...
//                    categoryIndex = inventory.keySet().size() - 1;
                    if (categoryIndex >= stock.keySet().size() - 1)
                        categoryIndex = stock.keySet().size() - 1;
                else
                    categoryIndex++;
            }
        }
...
private void updateStock(ItemStack[] array) {
    stock = new LinkedHashMap<>();
    
    List<ItemStack> items = Arrays.asList(array);
    // TODO: If stack is empty, show that it is empty!

    stock.put("All Items", items);
    for (ItemStack stack : items) {
        List<String> tabs = getTabsFromItem(stack);
        for(String tab : tabs) {
            stock.computeIfAbsent(tab, k -> new ArrayList<>()).add(stack);
        }
    }
}
